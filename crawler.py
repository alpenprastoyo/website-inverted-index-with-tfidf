...
class Crawler:
    ...
    def __init__(self, folder, base_url, domain_name):
        ...
        Crawler.pages_table = Crawler.folder + '/tables/'
        self.boot()
        ...
        try:
            ...
            if 'text/html' in response.getheader('Content-Type'):
                ...
                if Crawler.save_html_pages:
                    if tree.css_first('table') is None:
                        print('kosong')
                    else:
                        print('ADAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA')
                        soup = BeautifulSoup(tree.html)
                        [s.extract() for s in soup('style')]
                        for table in tree.css('table'):
                            htmltable = ""
                            htmltable = htmltable+table.html+'</br>'
                            code = Crawler.count_code
                            Crawler.count_code += 1
                            write_file(Crawler.pages_table+str(code), htmltable)
                            ...
                            Crawler.code_from_url += [[code,table.html,page_url]]
                            ...
                            with open('1. data.json', 'w') as outfile:
                                json.dump(Crawler.code_from_url, outfile)
